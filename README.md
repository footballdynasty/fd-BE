pg_dump -U <username> -h <connection_string> >> fd.sql

docker exec -i docker-compose-postgres-1 /bin/bash -c "PGPASSWORD=<password> psql --username fd_user fd_db" < fd.sql

Change the owner <username> to fd_user in the file before running the above command
